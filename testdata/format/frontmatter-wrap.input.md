...
Keep grammar-aware formatting as the primary lane for Markdown++ documents.
Preserve frontmatter while joining safe wrapped paragraphs into one line.
Repeated formatting must retain each continuation exactly once in the result.
Normal stdout mode must emit a complete document on every successful pass.
